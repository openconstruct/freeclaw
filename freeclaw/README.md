...
- `FREECLAW_WEB_UI_USERNAME` (default: `freeclaw`; username for optional HTTP Basic auth on dashboard routes)
- `FREECLAW_WEB_UI_PASSWORD` (optional; if set, `GET /` and `GET /api/system/metrics` require HTTP Basic auth)
...
- Memory: `memory_*` stores notes in a local SQLite DB. Use `memory_list` to enumerate recent entries (ids/keys), then `memory_get` for exact lookup. Memory is shared across CLI + Discord for the current project by default; override with `FREECLAW_MEMORY_DB`. Notes can be `pinned` or given a `ttl_seconds` expiry.
...
If you want the dashboard publicly reachable with authentication, set:

```bash
export FREECLAW_WEB_UI_PASSWORD='choose-a-strong-password'
# optional (default is "freeclaw")
export FREECLAW_WEB_UI_USERNAME='freeclaw'
```
...
- `python -m freeclaw skill package <name> [--output-dir dist/skills] [--zip] [--force]`
