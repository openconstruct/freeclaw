...
def _harden_runtime_artifact_perms() -> None:
    for d in [_bot_status_dir(), _token_history_dir()]:
        try:
...
def _ensure_and_check_authorized_user(
    ...
    *,
    ...
    once_enabled = _once_enabled(workspace)
    auth_id, auth_name = _load_authorized_user(workspace)

    # user.md is authoritative when present, even if once.md is already consumed.
    ...
    # No binding exists and one-time auth flow is not enabled.
    if not once_enabled:
        return True, None, None, False

    # once.md exists and user.md is missing: bind first observed author.
...
async def run_discord_bot(
    *,
    ...
    _harden_runtime_artifact_perms()
    ...
    discord_tools_cache: dict[str, Any] = {"base_id": None, "tools": None}
    ...
    def _discord_tools_builder() -> list[dict[str, Any]]:
        ...
        if not enable_tools or tool_ctx is None:
            return base
        base_id = id(base)
        cached_tools = discord_tools_cache.get("tools")
        if (
            discord_tools_cache.get("base_id") == base_id
            and isinstance(cached_tools, list)
        ):
        ...
        if _DISCORD_SEND_FILE_TOOL_NAME not in names:
            out.append(discord_send_file_tool)
        discord_tools_cache["base_id"] = base_id
        discord_tools_cache["tools"] = out
        return list(out)
    ...
    session_schema_lock = threading.Lock()
    session_schema_ready = {"value": False}
    ...
    def _db_connect() -> sqlite3.Connection:
        ...
        if not session_schema_ready["value"]:
            with session_schema_lock:
                if not session_schema_ready["value"]:
                    _init_session_schema(con)
                    session_schema_ready["value"] = True
        return con
