...
def run_agent(
    *,
    ...
    static_tools: list[dict[str, Any]] | None = None
    if tools_builder is None:
        static_tools = tools_override if tools_override is not None else (tool_schemas() if enable_tools else None)

    for _ in range(max_tool_steps + 1):
        tools = tools_builder() if tools_builder is not None else static_tools
        steps += 1
        try:
