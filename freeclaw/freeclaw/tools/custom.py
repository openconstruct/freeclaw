...
@dataclass(frozen=True)
class _CustomToolCacheEntry:
    signature: tuple[tuple[str, int, int], ...]
    specs: tuple[CustomToolSpec, ...]
    by_name: dict[str, CustomToolSpec]


_CUSTOM_TOOL_CACHE_LOCK = threading.Lock()
_CUSTOM_TOOL_CACHE: dict[str, _CustomToolCacheEntry] = {}
...
def _base_cache_key(base: Path) -> str:
    try:
...
def _spec_signature(spec_paths: list[Path]) -> tuple[tuple[str, int, int], ...]:
    ...
    for p in spec_paths:
        try:
            ...
            # If a file vanishes during scan, just skip it for this snapshot.
            continue
    return tuple(out)


def _load_specs_uncached(spec_paths: list[Path]) -> tuple[CustomToolSpec, ...]:
...
def _cached_specs_for_base(base: Path) -> tuple[CustomToolSpec, ...]:
    key = _base_cache_key(base)
    spec_paths = _discover_spec_paths(base)
    sig = _spec_signature(spec_paths)
    with _CUSTOM_TOOL_CACHE_LOCK:
        cached = _CUSTOM_TOOL_CACHE.get(key)
        if cached is not None and cached.signature == sig:
            return cached.specs

    specs = _load_specs_uncached(spec_paths)
    by_name = {s.name: s for s in specs}
    entry = _CustomToolCacheEntry(signature=sig, specs=specs, by_name=by_name)
    with _CUSTOM_TOOL_CACHE_LOCK:
        _CUSTOM_TOOL_CACHE[key] = entry
    return specs


def _cached_spec_by_name(base: Path, name: str) -> CustomToolSpec | None:
    key = _base_cache_key(base)
    spec_paths = _discover_spec_paths(base)
    sig = _spec_signature(spec_paths)
    with _CUSTOM_TOOL_CACHE_LOCK:
        cached = _CUSTOM_TOOL_CACHE.get(key)
        if cached is not None and cached.signature == sig:
            return cached.by_name.get(name)
...
def iter_custom_tools(ctx: ToolContext, *, reserved_names: set[str] | None = None) -> list[CustomToolSpec]:
    if not ctx.custom_tools_enabled:
        return []
    specs = list(_cached_specs_for_base(ctx.custom_tools_dir))
    if reserved_names:
        specs = [s for s in specs if s.name not in reserved_names]
    return specs
...
def dispatch_custom_tool_call(ctx: ToolContext, name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    spec = _cached_spec_by_name(ctx.custom_tools_dir, name)
    if spec is None:
        raise KeyError(name)
