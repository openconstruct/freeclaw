...
def run_onboarding(
    *,
    ...
    web_ui_auth_user = (os.getenv("FREECLAW_WEB_UI_USERNAME") or "freeclaw").strip() or "freeclaw"
    ...
    if web_ui_enabled:
        ...
        web_ui_auth_user = (
            _prompt(
                "Web UI username (HTTP Basic auth)",
                default=web_ui_auth_user,
            ).strip()
            or "freeclaw"
        )
        had_existing_web_ui_password = bool(web_ui_auth_password)
        pw_prompt = (
            "Web UI password (input hidden; blank = keep existing)"
            if had_existing_web_ui_password
            else "Web UI password (input hidden)"
        )
        while True:
            pw_in = getpass.getpass(f"{pw_prompt}: ").strip()
            if pw_in:
                web_ui_auth_password = pw_in
                break
            if had_existing_web_ui_password:
                break
            if _prompt_yes_no("No password entered. Leave Web UI unauthenticated?", default=False):
                web_ui_auth_password = ""
                break
        set_env_var(env_path, "FREECLAW_WEB_UI_USERNAME", web_ui_auth_user)
        set_env_var(env_path, "FREECLAW_WEB_UI_PASSWORD", web_ui_auth_password)
        ...
        sample_skill.write_text(
            "\n".join(
                [
                    "---",
                    "name: scaffold",
                    "description: Create or update project and skill scaffolds, including shareable skill packaging. Use when users ask for scaffolding, template setup, or packaging skills for distribution.",
                    "---",
                    "",
                    "# scaffold",
                    "",
                    "When creating or updating a skill scaffold:",
                    "- use a folder name like `<skill-name>` with lowercase letters, digits, and hyphens",
                    "- keep `SKILL.md` at the skill root",
                    "- include YAML frontmatter in `SKILL.md` with only `name` and `description`",
                    "- add optional folders only when needed: `scripts/`, `references/`, `assets/`, `tools/`",
                    "- if `tools/` is used, store specs as `<tool>.json` or `<tool>/tool.json`",
                    "- avoid unrelated files such as logs, caches, virtualenvs, and `.git`",
                    "",
                    "When asked to package a skill for sharing:",
                    "- create `dist/skills/<skill-name>/`",
                    "- copy only required files for that skill",
                    "- add `INSTALL.md` with:",
                    "  - copy `<skill-name>/` into a configured skills directory (default `config/skills`)",
                    "  - copy any custom tool specs from `<skill-name>/tools` into `<workspace>/skills/tools`",
                    "  - run `python -m freeclaw skill enable <skill-name>`",
                    "- create `<skill-name>.zip` only if the user asks for an archive",
                    "",
                ]
            )
            + "\n",
    ...
    if web_ui_enabled:
        auth_enabled = bool(web_ui_auth_password)
        if auth_enabled:
            print(f"- web ui auth: enabled (user {web_ui_auth_user})")
        else:
