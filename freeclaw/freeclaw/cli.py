...
from . import __version__ as FREECLAW_VERSION
...
Skill packaging (shareable):
- A shareable skill is a self-contained folder named `<skill-name>` with `SKILL.md` at the root.
- Keep `SKILL.md` concise and include YAML frontmatter with `name` and `description`.
- Optional skill folders: `scripts/`, `references/`, `assets/`, `tools/`.
- If a skill includes custom tools, store specs in `tools/` as `<name>.json` or `<name>/tool.json`.
- When asked to package/export/share a skill:
  - Prefer `python -m freeclaw skill package <skill-name> --zip --force` when shell execution is available.
  - Create `dist/skills/<skill-name>/` and copy only required files.
  - Exclude caches/build artifacts (`__pycache__`, virtualenvs, logs, `.git`, large temp files).
  - Add `INSTALL.md` with install steps:
    - copy `<skill-name>/` into a configured skills dir (default `config/skills`)
    - copy custom tool specs from `<skill-name>/tools` into `<workspace>/skills/tools`
    - run `python -m freeclaw skill enable <skill-name>`
  - If the user asks for an archive and shell tools are available, also produce `<skill-name>.zip`.
"""
...
_UPDATE_REPO_WEB_URL = "https://github.com/openconstruct/freeclaw"
_UPDATE_RELEASE_API_URL = "https://api.github.com/repos/openconstruct/freeclaw/releases/latest"
_UPDATE_TAGS_API_URL = "https://api.github.com/repos/openconstruct/freeclaw/tags?per_page=1"
_UPDATE_CHECK_TIMEOUT_S = 15.0
_UPDATE_REMINDER_DELAY_S = 3600
...
def _custom_tool_specs_signature(base: Path) -> tuple[tuple[str, int, int], ...]:
    try:
    ...
    for child in children:
        ...
        try:
            if child.is_file() and child.suffix.lower() == ".json":
                ...
                if tool_json.exists() and tool_json.is_file():
                    spec_path = tool_json
        except Exception:
            continue
        ...
        try:
            ...
            st = rp.stat()
            out.append((str(rp), int(st.st_mtime_ns), int(st.st_size)))
        except Exception:
            continue
...
def _build_tool_runtime(
    ...
    *,
    ...
    cached_sig: tuple[Any, ...] | None = None

    def tools_builder() -> list[dict[str, Any]]:
        nonlocal cached_tools, cached_sig
        custom_sig: tuple[tuple[str, int, int], ...]
        if tool_ctx.custom_tools_enabled:
            custom_sig = _custom_tool_specs_signature(tool_ctx.custom_tools_dir)
        else:
            custom_sig = ()
        sig = (bool(tool_ctx.shell_enabled), bool(tool_ctx.custom_tools_enabled), custom_sig)
        if cached_tools is not None and cached_sig == sig:
            return cached_tools
...
def _update_check_state_path(workspace: Path) -> Path:
    return workspace / "mem" / "update_check_state.json"


def _update_md_path(workspace: Path) -> Path:
    return workspace / "update.md"


def _read_update_check_state(path: Path) -> dict[str, Any]:
    try:
        if not path.exists() or not path.is_file():
            return {}
...
def _save_update_check_state(path: Path, state: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
...
def _is_remote_version_newer(*, remote_version: str, local_version: str) -> bool:
    rv = _version_tuple(remote_version)
    lv = _version_tuple(local_version)
    if rv is None or lv is None:
        return False
    return rv > lv


def _fetch_github_latest_version() -> tuple[dict[str, str] | None, str | None]:
    headers = {
        "Accept": "application/vnd.github+json",
        "User-Agent": f"freeclaw/{str(FREECLAW_VERSION).strip() or '0.1.0'}",
    }
    release_err: str | None = None
    try:
        rel = get_json(
            _UPDATE_RELEASE_API_URL,
            headers=headers,
        ...
        if isinstance(rel, dict):
            ...
            if tag:
                rel_url = str(rel.get("html_url") or "").strip() or _UPDATE_REPO_WEB_URL
                return {
                    "version": tag,
                    "source": "release",
                    "url": rel_url,
        ...
        if isinstance(tags, list):
            for row in tags:
                ...
                return {
                    "version": tag,
                    "source": "tag",
                    "url": _UPDATE_REPO_WEB_URL,
                }, None
        tags_err = "no tags returned"
    except Exception as e:
        tags_err = str(e)

    err_parts = [x for x in [release_err, tags_err] if x]
    if not err_parts:
        return None, "no version data returned from GitHub"
    return None, "; ".join(err_parts)[:500]


def _render_update_md(
    *,
    local_version: str,
    remote_version: str,
    source: str,
    ...
    detected_at = dt.datetime.fromtimestamp(int(detected_at_s), tz=dt.timezone.utc).astimezone().isoformat(
        timespec="seconds"
    )
    remind_at = dt.datetime.fromtimestamp(int(remind_at_s), tz=dt.timezone.utc).astimezone().isoformat(
        timespec="seconds"
    )
    checked_at = dt.datetime.fromtimestamp(int(checked_at_s), tz=dt.timezone.utc).astimezone().isoformat(
        timespec="seconds"
    )
    return (
        "\n".join(
            [
                "# update",
                "",
                "A newer Freeclaw version is available on GitHub.",
                "",
                f"- repo: {_UPDATE_REPO_WEB_URL}",
                f"- local_version: {str(local_version).strip() or '(unknown)'}",
                f"- latest_version: {str(remote_version).strip() or '(unknown)'}",
                f"- source: {str(source).strip() or '(unknown)'}",
                f"- detected_at: {detected_at}",
                f"- reminder_at: {remind_at}",
                f"- last_checked_at: {checked_at}",
                "",
                "Upgrade steps:",
                "1. Pull latest code from GitHub.",
                "2. Reinstall dependencies if needed (`pip install -r requirements.txt`).",
                "3. Restart freeclaw services (timer-api/discord/chat).",
                "",
                f"Reference: {str(update_url).strip() or _UPDATE_REPO_WEB_URL}",
                "",
            ]
        )
        + "\n"
...
def _task_timer_check_for_updates(runtime: _TaskTimerRuntime, *, now_s: int) -> dict[str, Any]:
    workspace = runtime.workspace
    state_path = _update_check_state_path(workspace)
    update_md_path = _update_md_path(workspace)
    state = _read_update_check_state(state_path)

    interval_s = max(60, int(max(1, runtime.minutes)) * 60)
    try:
        last_checked_s = int(state.get("last_checked_s") or 0)
    except Exception:
        last_checked_s = 0
    should_check = (int(now_s) - int(last_checked_s)) >= int(interval_s)

    if should_check:
        latest, check_err = _fetch_github_latest_version()
        state["last_checked_s"] = int(now_s)
        state["check_interval_s"] = int(interval_s)
        state["last_error"] = (None if not check_err else str(check_err)[:500])
        local_version = str(FREECLAW_VERSION).strip() or "0.0.0"

        if latest is not None:
            remote_version = str(latest.get("version") or "").strip()
            source = str(latest.get("source") or "").strip() or "unknown"
            update_url = str(latest.get("url") or "").strip() or _UPDATE_REPO_WEB_URL
            if remote_version and _is_remote_version_newer(
                remote_version=remote_version,
                local_version=local_version,
            ):
                prev_remote = str(state.get("remote_version") or "").strip()
                was_available = bool(state.get("update_available", False))
                try:
                    detected_at_s = int(state.get("detected_at_s") or 0)
                except Exception:
                    detected_at_s = 0
                if (not was_available) or (prev_remote != remote_version) or detected_at_s <= 0:
                    detected_at_s = int(now_s)
                    remind_at_s = int(now_s) + int(_UPDATE_REMINDER_DELAY_S)
                    reminder_sent = False
                    reminded_at_s: int | None = None
                else:
                    try:
                        remind_at_s = int(state.get("remind_at_s") or 0)
                    except Exception:
                        remind_at_s = 0
                    if remind_at_s <= 0:
                        remind_at_s = int(detected_at_s) + int(_UPDATE_REMINDER_DELAY_S)
                    reminder_sent = bool(state.get("reminder_sent", False))
                    try:
                        reminded_at_s = int(state.get("reminded_at_s")) if state.get("reminded_at_s") is not None else None
                    except Exception:
                        reminded_at_s = None
                ...
                state.update(
                    {
                        ...
                        "source": source,
                        "url": update_url,
                        "detected_at_s": int(detected_at_s),
                        "remind_at_s": int(remind_at_s),
                        "reminder_sent": bool(reminder_sent),
                        "reminded_at_s": reminded_at_s,
                    }
                )
                update_md_path.parent.mkdir(parents=True, exist_ok=True)
                ...
                try:
                    update_md_path.unlink(missing_ok=True)
                except Exception:
                    pass
        _save_update_check_state(state_path, state)

    state = _read_update_check_state(state_path) or state
    update_available = bool(state.get("update_available", False))
    remote_version = str(state.get("remote_version") or "").strip() or None
    local_version = str(state.get("local_version") or "").strip() or (str(FREECLAW_VERSION).strip() or "0.0.0")
    source = str(state.get("source") or "").strip() or None
    url = str(state.get("url") or "").strip() or _UPDATE_REPO_WEB_URL
    try:
        detected_at_s = int(state.get("detected_at_s")) if state.get("detected_at_s") is not None else None
    except Exception:
        detected_at_s = None
    try:
        remind_at_s = int(state.get("remind_at_s")) if state.get("remind_at_s") is not None else None
    except Exception:
        remind_at_s = None
    reminder_sent = bool(state.get("reminder_sent", False))
    try:
    ...
    reminder_due = bool(
        ...
        and remind_at_s is not None
        and int(now_s) >= int(remind_at_s)
    )
    try:
        out_last_checked_s = int(state.get("last_checked_s") or 0)
    except Exception:
        out_last_checked_s = 0
    err = str(state.get("last_error") or "").strip() or None
    return {
        "checked": bool(should_check),
        "check_interval_s": int(interval_s),
        "last_checked_s": int(out_last_checked_s),
        ...
        "remind_at_s": remind_at_s,
        "reminder_due": bool(reminder_due),
        "reminder_sent": bool(reminder_sent),
        "reminded_at_s": reminded_at_s,
        "state_path": str(state_path),
        "update_md_path": str(update_md_path),
        "error": err,
    }


def _task_timer_mark_update_reminder_sent(workspace: Path, *, now_s: int) -> None:
    state_path = _update_check_state_path(workspace)
    state = _read_update_check_state(state_path)
    if not state:
        return
    if not bool(state.get("update_available", False)):
        return
    state["reminder_sent"] = True
    state["reminded_at_s"] = int(now_s)
    _save_update_check_state(state_path, state)
...
def _read_update_md(workspace: Path) -> str | None:
    p = workspace / "update.md"
    try:
        if not p.exists() or not p.is_file():
            return None
        data = p.read_bytes()
...
def _core_system_prelude(
    ...
    *,
    ...
    update_txt = _read_update_md(workspace)
    if update_txt:
        parts.append("Update notice (from update.md):\n" + update_txt)
...
def _truncate_chat_messages_in_place(messages: list[dict[str, Any]], *, history_messages: int) -> None:
    limit = max(1, int(history_messages))
    ...
    for m in messages:
        ...
        role = str(m.get("role") or "").strip().lower()
        content = m.get("content")
        if role == "system" and system_msg is None and isinstance(content, str):
    ...
    if len(tail) > limit:
        tail = tail[-limit:]
    messages[:] = ([system_msg] if system_msg is not None else []) + tail
...
def cmd_chat(args: argparse.Namespace) -> int:
    ...
    if system_prompt:
        messages.append({"role": "system", "content": system_prompt})
    chat_history_messages_raw = str(os.getenv("FREECLAW_CHAT_HISTORY_MESSAGES") or "").strip()
    try:
        chat_history_messages = int(chat_history_messages_raw) if chat_history_messages_raw else 40
    except Exception:
        chat_history_messages = 40
    chat_history_messages = max(2, min(200, int(chat_history_messages)))
    ...
    while True:
        ...
        _truncate_chat_messages_in_place(messages, history_messages=chat_history_messages)
        ...
        if not result.text.endswith("\n"):
            sys.stdout.write("\n")
        assistant_text = str(result.text or "").strip() or "(no response)"
...
def _task_timer_tick(runtime: _TaskTimerRuntime) -> dict[str, Any]:
    ...
    update_info = _task_timer_check_for_updates(runtime, now_s=now_s)
    items = _iter_tasks(tasks_p)
    due, summary = _compute_due_tasks(items=items, last_run=last_run, now_s=now_s)
    update_reminder_due = bool(update_info.get("reminder_due", False))
    if update_reminder_due:
        remote_version = str(update_info.get("remote_version") or "").strip() or "latest"
        local_version = str(update_info.get("local_version") or "").strip() or (str(FREECLAW_VERSION).strip() or "local")
        try:
            detected_at_s = int(update_info.get("detected_at_s") or now_s)
        except Exception:
            detected_at_s = int(now_s)
        elapsed_m = max(0, int(now_s - detected_at_s) // 60)
        reminder_item = _TaskItem(
            raw=f"60-remind the user to upgrade freeclaw ({local_version} -> {remote_version})",
            checked=False,
            dotime_minutes=60,
            task=(
                "Remind the user to upgrade freeclaw. "
                f"A newer version is available on GitHub ({local_version} -> {remote_version})."
            ),
            key=f"system:update-reminder:{remote_version}",
        )
        due.append(
            _DueTask(
                item=reminder_item,
                last_run_s=None,
                elapsed_minutes=int(elapsed_m),
            )
        )
    ...
    for d in due:
        ...
        if str(it.key).startswith("system:update-reminder:"):
            row["event"] = "update_reminder"
        due_json.append(row)

    summary["due"] = int(len(due))
    ...
    out: dict[str, Any] = {
        ...
        "due_tasks": due_json,
        "update_check": {
            "checked": bool(update_info.get("checked", False)),
            "check_interval_s": int(update_info.get("check_interval_s", 0) or 0),
            "last_checked_s": int(update_info.get("last_checked_s", 0) or 0),
            "update_available": bool(update_info.get("update_available", False)),
            "local_version": (None if not str(update_info.get("local_version") or "").strip() else str(update_info.get("local_version")).strip()),
            "remote_version": (None if not str(update_info.get("remote_version") or "").strip() else str(update_info.get("remote_version")).strip()),
            "source": (None if not str(update_info.get("source") or "").strip() else str(update_info.get("source")).strip()),
            "url": str(update_info.get("url") or _UPDATE_REPO_WEB_URL),
            "detected_at_s": (
                None
                if update_info.get("detected_at_s") is None
                else int(update_info.get("detected_at_s"))
            ),
            "remind_at_s": (
                None
                if update_info.get("remind_at_s") is None
                else int(update_info.get("remind_at_s"))
            ),
            "reminder_due": bool(update_info.get("reminder_due", False)),
            "reminder_sent": bool(update_info.get("reminder_sent", False)),
            "reminded_at_s": (
                None
                if update_info.get("reminded_at_s") is None
                else int(update_info.get("reminded_at_s"))
            ),
            "state_path": str(update_info.get("state_path") or _update_check_state_path(runtime.workspace)),
            "update_md_path": str(update_info.get("update_md_path") or _update_md_path(runtime.workspace)),
            "error": (None if not str(update_info.get("error") or "").strip() else str(update_info.get("error")).strip()),
        },
    }
    ...
    if result_ok:
        ...
        if update_reminder_due:
            _task_timer_mark_update_reminder_sent(runtime.workspace, now_s=now_s)
            uo = out.get("update_check")
            if isinstance(uo, dict):
                uo["reminder_due"] = False
                uo["reminder_sent"] = True
                uo["reminded_at_s"] = int(now_s)
        out["ran"] = True
    else:
        out["ok"] = False
        out["ran"] = False
...
def _safe_read_text_range(path: Path, *, start: int, max_bytes: int) -> str | None:
    try:
        with path.open("rb") as f:
            f.seek(max(0, int(start)))
            b = f.read(max(0, int(max_bytes)))
    except Exception:
        return None
    return b.decode("utf-8", errors="replace")
...
@dataclass(frozen=True)
class _TokenUsageFileCache:
    size: int
    mtime_ns: int
    events: tuple[_TokenUsageEvent, ...]


_TOKEN_USAGE_FILE_CACHE_LOCK = threading.Lock()
_TOKEN_USAGE_FILE_CACHE: dict[str, _TokenUsageFileCache] = {}
...
def _parse_token_history_lines(txt: str) -> tuple[_TokenUsageEvent, ...]:
    if not txt:
        return ()
    events: list[_TokenUsageEvent] = []
    for ln in txt.splitlines():
        ...
        events.append(_TokenUsageEvent(ts=float(ts), bot_label=label, total_tokens=int(total)))
    return tuple(events)


def _parse_token_history_events(path: Path) -> tuple[_TokenUsageEvent, ...]:
    txt = _safe_read_text(path, max_bytes=5_000_000)
    if not txt:
        return ()
    return _parse_token_history_lines(txt)


def _cached_token_history_events(path: Path) -> tuple[_TokenUsageEvent, ...]:
    try:
        st = path.stat()
    except Exception:
        return ()
    ...
    sig_mtime = int(st.st_mtime_ns)
    with _TOKEN_USAGE_FILE_CACHE_LOCK:
        cached = _TOKEN_USAGE_FILE_CACHE.get(key)
        if cached is not None and cached.size == sig_size and cached.mtime_ns == sig_mtime:
            return cached.events
        cached_for_append = cached

    # Fast path: file was append-only since the last parse.
    if (
        cached_for_append is not None
        and sig_size >= cached_for_append.size
        and sig_mtime >= cached_for_append.mtime_ns
    ):
        delta = _safe_read_text_range(
            path,
            start=int(cached_for_append.size),
            max_bytes=max(0, int(sig_size - cached_for_append.size)),
        )
        if delta is not None:
            appended = _parse_token_history_lines(delta)
            events = cached_for_append.events + appended
            fresh = _TokenUsageFileCache(size=sig_size, mtime_ns=sig_mtime, events=events)
            with _TOKEN_USAGE_FILE_CACHE_LOCK:
                _TOKEN_USAGE_FILE_CACHE[key] = fresh
            return fresh.events

    # Fallback: full parse when file rotated/truncated or append fast-path failed.
    events = _parse_token_history_events(path)
    fresh = _TokenUsageFileCache(size=sig_size, mtime_ns=sig_mtime, events=events)
    with _TOKEN_USAGE_FILE_CACHE_LOCK:
        _TOKEN_USAGE_FILE_CACHE[key] = fresh
    return events
...
def _build_bot_token_metrics(*, now_s: float, days: int = 7) -> dict[str, Any]:
    ...
    token_paths = _token_history_paths(now_s=now_s, days=days)
    token_path_keys = {str(p) for p in token_paths}
    with _TOKEN_USAGE_FILE_CACHE_LOCK:
        stale_keys = [k for k in _TOKEN_USAGE_FILE_CACHE.keys() if k not in token_path_keys]
        for k in stale_keys:
            _TOKEN_USAGE_FILE_CACHE.pop(k, None)
    for p in token_paths:
        for ev in _cached_token_history_events(p):
            if ev.ts < cutoff_7d:
                continue
            b = by_label.setdefault(ev.bot_label, {"t24": 0, "t7d": 0})
            b["t7d"] = int(b.get("t7d", 0) + int(ev.total_tokens))
            if ev.ts >= cutoff_24h:
                b["t24"] = int(b.get("t24", 0) + int(ev.total_tokens))
...
def cmd_timer_api(args: argparse.Namespace) -> int:
    ...
    web_ui_auth_user = (str(os.getenv("FREECLAW_WEB_UI_USERNAME") or "").strip() or "freeclaw")
    web_ui_auth_password = str(os.getenv("FREECLAW_WEB_UI_PASSWORD") or "").strip()
    web_ui_auth_enabled = bool(web_ui_auth_password)
    log.info(
        "timer-api start host=%s port=%d poll_seconds=%.1f web_ui=%s web_ui_auth=%s mode=%s targets=%d primary_workspace=%s",
        host,
        int(port),
        float(poll_seconds),
        bool(web_ui_enabled),
        bool(web_ui_auth_enabled),
        ("all-agents" if include_all_agents else "single"),
        int(len(targets)),
        str(runtime.workspace),
    )
    log.info(
        "timer-api google oauth callback configured=%s db_count=%d",
        bool(google_redirect_uri),
        int(len(oauth_db_paths)),
    )
    ...
    metrics_cache_ttl_raw = str(os.getenv("FREECLAW_WEB_METRICS_CACHE_S") or "").strip()
    try:
        metrics_cache_ttl_s = float(metrics_cache_ttl_raw) if metrics_cache_ttl_raw else 2.0
    except Exception:
        metrics_cache_ttl_s = 2.0
    if metrics_cache_ttl_s < 0:
        metrics_cache_ttl_s = 0.0
    if metrics_cache_ttl_s > 60.0:
        metrics_cache_ttl_s = 60.0
    metrics_cache_lock = threading.Lock()
    metrics_cache: dict[str, Any] = {"ts": 0.0, "payload": None}
    enabled = {"value": True}
    status: dict[str, Any] = {
        "last_tick_at": None,
        "last_result": None,
    ...
    def _cached_system_metrics() -> dict[str, Any]:
        ...
        if metrics_cache_ttl_s > 0:
            with metrics_cache_lock:
                cached_ts = float(metrics_cache.get("ts", 0.0) or 0.0)
                cached_payload = metrics_cache.get("payload")
                if isinstance(cached_payload, dict) and (now_s - cached_ts) < metrics_cache_ttl_s:
    ...
    class _TimerHTTPServer(ThreadingHTTPServer):
        daemon_threads = True
        ...
        def handle_error(self, request: Any, client_address: Any) -> None:
            ...
            if isinstance(exc, (BrokenPipeError, ConnectionResetError, ConnectionAbortedError)):
                try:
                    peer = str((client_address or ("", 0))[0] or "")
                except Exception:
                    peer = ""
                if peer:
                    log.debug("timer-api client disconnected peer=%s error=%s", peer, type(exc).__name__)
                return
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def log_message(self, fmt: str, *args: Any) -> None:  # noqa: A003
            msg = str(fmt % args)
            msg = re.sub(
                r'"([A-Z]+)\s+([^"\s?]+)\?[^"\s]*(\s+HTTP/[0-9.]+)"',
                r'"\1 \2?<redacted>\3"',
                msg,
            )
            msg = self._sanitize_log_text(msg)
            log.info("timer-api %s - %s", self.address_string(), msg)
        ...
        def _body_allowed(self) -> bool:
            return str(getattr(self, "command", "")).upper() != "HEAD"
        ...
        def _send_auth_required(self) -> None:
            ...
            self.send_header("WWW-Authenticate", 'Basic realm="freeclaw-dashboard", charset="UTF-8"')
        ...
        def _check_web_ui_auth(self) -> bool:
            if not web_ui_auth_enabled:
                return True
            ...
            try:
                raw = base64.b64decode(token, validate=True).decode("utf-8", errors="replace")
            except Exception:
                self._send_auth_required()
                return False
            ...
            if not sep:
                self._send_auth_required()
                return False
            if not hmac.compare_digest(user, web_ui_auth_user):
                self._send_auth_required()
                return False
            if not hmac.compare_digest(pw, web_ui_auth_password):
                self._send_auth_required()
                return False
            return True
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            if p in {"/", "/ui"}:
                if not web_ui_enabled:
                    ...
                    return
                if not self._check_web_ui_auth():
                    return
            ...
            if p == "/api/system/metrics":
                ...
                if not self._check_web_ui_auth():
                    return
                self._send_json(200, _cached_system_metrics())
                return
            if p == "/timer/status":
                ...
                with state_lock:
                    payload = {
                        ...
                        "web_ui_auth_enabled": bool(web_ui_auth_enabled),
                        "web_metrics_cache_seconds": float(metrics_cache_ttl_s),
                        "workspace": str(runtime.workspace),
                        "tasks_path": str(runtime.workspace / "tasks.md"),
    ...
    srv = _TimerHTTPServer((host, port), _Handler)
    tasks_str = (
...
_SKILL_PACKAGE_EXCLUDE_FILE_NAMES = {
    ".ds_store",
}
_SKILL_PACKAGE_EXCLUDE_SUFFIXES = {
    ".log",
    ".tmp",
    ".bak",
    ".swp",
...
def _skill_package_should_exclude(rel: Path, *, is_dir: bool) -> bool:
    parts = [part.lower() for part in rel.parts if part and part != "."]
    if any(part in _SKILL_PACKAGE_EXCLUDE_NAMES for part in parts):
        return True
    if is_dir:
        return False
    name = rel.name.lower()
    if name in _SKILL_PACKAGE_EXCLUDE_FILE_NAMES:
        return True
    return any(name.endswith(suffix) for suffix in _SKILL_PACKAGE_EXCLUDE_SUFFIXES)


def _copy_skill_tree_for_package(src: Path, dst: Path) -> tuple[int, int, int]:
    copied_files = 0
    copied_bytes = 0
    skipped_symlinks = 0
    for root, dirnames, filenames in os.walk(src):
        ...
        for dirname in sorted(dirnames):
            ...
            if _skill_package_should_exclude(rel_dir, is_dir=True):
                continue
        ...
        for filename in sorted(filenames):
            ...
            if _skill_package_should_exclude(rel_file, is_dir=False):
                continue
            src_file = root_p / filename
            ...
            copied_files += 1
            try:
                copied_bytes += int(src_file.stat().st_size)
            except Exception:
                pass
    return copied_files, copied_bytes, skipped_symlinks


def _render_skill_install_md(skill_name: str) -> str:
    return (
        "\n".join(
            [
                f"# Install `{skill_name}`",
                "",
                "## 1) Install skill folder",
                "Copy this folder into a configured skills directory.",
                "Default destination:",
                f"- `config/skills/{skill_name}`",
                "",
                "## 2) Install custom tools (optional)",
                f"If `{skill_name}/tools/` exists, copy tool specs into your workspace tools directory:",
                ...
                "## 3) Enable the skill",
                "```bash",
                f"python -m freeclaw skill enable {skill_name}",
                "```",
                "",
                "## 4) Verify",
                "```bash",
...
def cmd_skill_package(args: argparse.Namespace) -> int:
    ...
    src_dir = sk.path.resolve()
    output_dir = _resolve_cli_path(str(getattr(args, "output_dir", "dist/skills")))
    package_dir = (output_dir / sk.name).resolve()

    try:
        package_dir.relative_to(src_dir)
        raise SystemExit("Output directory cannot be inside the source skill directory.")
    except ValueError:
        pass

    if package_dir.exists():
        if not bool(getattr(args, "force", False)):
            raise SystemExit(f"Package directory already exists: {package_dir} (use --force to overwrite)")
        shutil.rmtree(package_dir)

    output_dir.mkdir(parents=True, exist_ok=True)
    package_dir.mkdir(parents=True, exist_ok=True)

    copied_files, copied_bytes, skipped_symlinks = _copy_skill_tree_for_package(src_dir, package_dir)

    install_md = package_dir / "INSTALL.md"
    install_md.write_text(_render_skill_install_md(sk.name), encoding="utf-8")
    copied_files += 1
    try:
        copied_bytes += int(install_md.stat().st_size)
    except Exception:
        pass

    archive_path: Path | None = None
    if bool(getattr(args, "zip", False)):
        archive_path = (output_dir / f"{sk.name}.zip").resolve()
        if archive_path.exists():
            if not bool(getattr(args, "force", False)):
                raise SystemExit(f"Archive already exists: {archive_path} (use --force to overwrite)")
            archive_path.unlink()
        archive_base = (output_dir / sk.name).resolve()
        shutil.make_archive(str(archive_base), "zip", root_dir=str(output_dir), base_dir=sk.name)

    sys.stdout.write(f"packaged skill: {sk.name}\n")
    sys.stdout.write(f"- source: {src_dir}\n")
    sys.stdout.write(f"- output: {package_dir}\n")
    if archive_path is not None:
        sys.stdout.write(f"- archive: {archive_path}\n")
    sys.stdout.write(f"- files: {copied_files}\n")
    sys.stdout.write(f"- bytes: {copied_bytes}\n")
    if skipped_symlinks:
        sys.stdout.write(f"- skipped symlinks: {skipped_symlinks}\n")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = build_main_parser(
        handlers={
            ...
            "cmd_skill_package": cmd_skill_package,
        }
    )
