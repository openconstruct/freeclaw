...
def build_main_parser(*, handlers: dict[str, Any]) -> argparse.ArgumentParser:
    ...
    p_skill = sub.add_parser("skill", help="Skills: list/show/enable/disable/package")
    ...
    p_s_pkg = sub_skill.add_parser("package", help="Package a skill for sharing.")
    p_s_pkg.add_argument("name", help="Skill name to package.")
    p_s_pkg.add_argument(
        "--output-dir",
        default="dist/skills",
        help="Output directory for packaged skills (default: ./dist/skills).",
    )
    p_s_pkg.add_argument(
        "--zip",
        action="store_true",
        help="Also create <name>.zip alongside the packaged folder.",
    )
    p_s_pkg.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing package folder/archive if present.",
    )
    p_s_pkg.set_defaults(func=_handler(handlers, "cmd_skill_package"))
