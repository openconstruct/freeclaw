...
def setup_logging(*, level: str | None = None, log_file: str | None = None, log_format: str | None = None) -> None:
    ...
    if lf:
        ...
        # Keep log artifacts private to the local account when possible.
        for cand in [p, p.with_suffix(p.suffix + ".1"), p.with_suffix(p.suffix + ".2"), p.with_suffix(p.suffix + ".3")]:
            _chmod_owner_only(cand)
